...
def profile_view(request, slug):
    ...
    auctions = Auction.objects.filter(createdBy=u)
...
def profile_listings(request, slug):
    user = Profile.objects.filter(slug=slug).first()
    auctions_by_user = Auction.objects.filter(createdBy=user.user)
    max_bids_dict = {}
    for auction in auctions_by_user:
    ...
    return render(request, "accounts/profile_listings.html", {
        'user': user.user,
        'auctions': auctions_by_user,
...
def my_profile(request):
    ...
    friends = p.friends.all()
    auctions = Auction.objects.filter(createdBy=you)
